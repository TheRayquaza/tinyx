...
public interface SrvcHomeTimelineControllerApi extends Logger {
    ...
    @APIResponses({
            @APIResponse(responseCode = "200", description = "Get a list of posts related to a specific user's follows", content = @Content(schema = @Schema(implementation = HomeTimelineEntity.class))),
            @APIResponse(responseCode = "400", description = "Invalid Input"),
            @APIResponse(responseCode = "401", description = "User not authorized"),
            @APIResponse(responseCode = "404", description = "User not Found"),
    })
    @NotNull
    HomeTimelineEntity getHomeTimelineById(@PathParam("id") String id);
}
