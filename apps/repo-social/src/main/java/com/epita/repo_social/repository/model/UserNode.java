...
public record UserNode(
  ...
  public String findCypher() {
    return String.format("MATCH (u:User{userId:\"%s\"}) RETURN u", this.userId);
  }
  ...
  public String deleteCypher() {
    return String.format(
        "MATCH (n:User {userId: \"%s\"}) SET n.deleted = true RETURN n", this.userId);
  }

  public String getLikedPostsCypher() {
    return String.format(
        "MATCH (n:User {userId:\"%s\"})-[:HAS_LIKED]->(p:Post) RETURN p", this.userId);
  }

  public String getFollowersCypher() {
    return String.format(
        "MATCH (n:User {userId:\"%s\"})<-[:HAS_FOLLOWED]-(u:User) RETURN u", this.userId);
  }
  ...
  public String getBlockedCypher() {
    return String.format(
        "MATCH (:User {userId:\"%s\"})-[:HAS_BLOCKED]->(u:User) RETURN u", this.userId);
  }

  public String getBlockedByCypher() {
    return String.format(
        "MATCH (n:User {userId:\"%s\"})<-[:HAS_BLOCKED]-(u:User) RETURN u", this.userId);
  }

  public String deleteRelationshipCypher() {
    return String.format("MATCH (:User {userId:\"%s\"})-[r]-() DELETE r", this.userId);
  }
}
