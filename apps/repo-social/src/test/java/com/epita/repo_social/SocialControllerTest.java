...
class RepoSocialControllerTest {
  ...
  void unlikePost() {
    ...
    boolean relationExists =
        neo4jRepository.checkRelationExists(
            "MATCH (u:User {userId: \""
                + USER_ID_1
                + "\"})-[r:LIKES]->(p:Post {postId: \""
                + POST_ID_2
                + "\"}) RETURN r");
  ...
  void unblockUser() {
    ...
    boolean relationExists =
        neo4jRepository.checkRelationExists(
            "MATCH (u1:User {userId: \""
                + USER_ID_1
                + "\"})-[r:BLOCKS]->(u2:User {userId: \""
                + USER_ID_2
                + "\"}) RETURN r");
    assert !relationExists;
  }
  ...
  void getUsersWhoLikedPostEmpty() {
    ...
    System.out.println(response.body().prettyPrint());
    response.then().statusCode(200).body("size()", is(0)).extract().response();
  }
  ...
  void blockingDeletesFollowRelationship() {
    ...
    responseFollower.then().statusCode(200).body("size()", is(0)).extract().response();
    ...
    Response responseFollowing =
        given()
            ...
            .extract()
            .response();
    responseFollowing.then().statusCode(200).body("size()", is(0)).extract().response();
  }
  ...
  void DeletedPostBehavior() {
    ...
    socialService.createOrUpdatePostFromAggregate(
        PostAggregate.builder().build().withId(POST_ID_1).withOwnerId(USER_ID_1).withDeleted(true));

    // Vérifier que le like a été supprimé
    List<UserNode> likes =
        neo4jRepository.getUsers(
            """
  ...
  void deleteUserBehavior() {
    ...
    // delete user-1
    socialService.createOrUpdateUserFromAggregate(
